    ...
    <div className={s.itemHeader}>
      <div className={s.dateAndTaskStatus}>
        ...
        <div className={s.totalAndCompletedTask}>
          ...
          <span
            ...
            {
              t.taskContent.length
                ? t.taskContent.filter(tc => tc.completed).length
                : 0
            }
            </b>
          </span>
      ...
      <div className={s.action}>
        <div className={s.addTask}
             onClick={() => addTaskId(t.id, t.dateForPlane)}>
          Добавить
        </div>
        <div className={!t.editStatus ? s.openList : (s.openList + ' ' + s.active)}
             onClick={() => {
        ...
        <div className={s.buttonsDell}>
          <div className={s.dellFinishedTask}
               onClick={() => removeAllCompletedTaskToContent(t.id)}>
            Удалить завершенные
          </div>
          <div className={s.dellAllTask}
               onClick={() => removeAllThisTaskContent(t.id)}>
            Удалить все
          </div>
        </div>
      </div>

			...
			<div className={s.calendarWrapper} onClick={(event)=>event.stopPropagation()}>
				...
				<div className={s.activeDate}>
					{this.props.tasks.map((t: Task)=>
						<div className={s.dateItem} onClick={()=>this.onChangeDate(t.dateForPlane)} key={t.id}>
							<span>Дата : <b>{t.dateForPlane}</b> </span>
							<span>Всего : <b>{t.taskContent.length}</b> </span>
							<span>Завершено : <b>{t.taskContent.filter((tc: TaskContent)=>tc.completed).length}</b> </span>
						</div>
					)}
				</div>
			</div>

...
const SET_NEW_DATE_FOR_TASK: string = '/todoReducer___SET_NEW_DATE_FOR_TASK';
const NEW_NAME_AND_DESCRIPTION: string = '/todoReducer___NEW_NAME_AND_DESCRIPTION';
const REMOVE_ALL_COMPLETED_TASK: string = '/todoReducer___REMOVE_ALL_COMPLETED_TASK';
...
const todoTsReducer = (state = initialState, action: IAction) => {
  switch (action.type) {
    ...
    case REMOVE_ALL_COMPLETED_TASK: {
      const content = state.tasks.map((t: Task) => {
        removeByAttr(t.taskContent, 'completed', true);
        return t.taskContent
      });
      return {
        ...state, ...state.tasks = state.tasks.filter((t: Task) => t.taskContent), ...state.tasks.taskContent = content
...
const setNewTask = (newTask: Task) => (
  {type: ADD_NEW_TASK, newTask}
);

const setToggleShowTaskContent = (idAndStatus: any) => (
  {
    type: TOGGLE_SHOW_TASK_CONTENT, ...idAndStatus
  }
);

const setNewDateForTask = (idAndDate: any) => (
  {
    type: SET_NEW_DATE_FOR_TASK, ...idAndDate
  }
);
...
const setRemoveTaskContent = (idContent: number) => (
  {
    type: REMOVE_TASK_CONTENT, idContent
  }
);

const setRemoveAllTaskContent = (id: number | undefined) => (
  {
    type: REMOVE_ALL_TASK_CONTENT, id
  }
);

const setStatusCompletedTask = (idAndStatus: any) => (
  {
    type: SET_STATUS_COMPLETED_TASK, ...idAndStatus
  }
);

const setRemoveCompletedTaskToContent = (id: number | undefined) => (
  {
    type: REMOVE_COMPLETED_TASK_TO_CONTENT, id
  }
);

const setChangeNameAndDescription = (newContent: any) => (
  {
    type: NEW_NAME_AND_DESCRIPTION, ...newContent
  }
);

export const setRemoveAllCompletedTask = () => (
  {
    type: REMOVE_ALL_COMPLETED_TASK
  }
);
...
export const toggleShowTaskContent: (id: number | undefined, status: boolean | undefined) => {} =
  (id: number | undefined, status: boolean | undefined) => {
    return (dispatch: (p: any) => {}) => {
      dispatch(setToggleShowTaskContent({id, status}));
    }
  };

export const newDateForTask: (id: number | null, date: string | undefined) => {} =
  (id: number | null, date: string | undefined) => {
    return (dispatch: (p: any) => {}) => {
      dispatch(setNewDateForTask({id, date}));
    }
  };

export const addNewTaskContent: (idContent: number | null, newTaskContent: TaskContent) => {} =
  (idContent: number | null, newTaskContent: TaskContent) => {
    return (dispatch: (p: any) => {}) => {
...
export const toSetStatusCompletedTask: (id: number | undefined, status: boolean | undefined) => {} =
  (id: number | undefined, status: boolean | undefined) => {
    return (dispatch: (p: any) => {}) => {
...
export const changeNameAndDescription: (idContent: number | null | undefined, name: string, description: string) => {} =
  (idContent: number | null | undefined, name: string, description: string) => {
    return (dispatch: (p: any) => {}) => {
